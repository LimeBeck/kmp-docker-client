...
- Each operation has an overload with a required suspending `onProgress(ImageProgress)` callback; existing signatures remain available. Non-error records are delivered in wire order, without an internal queue, awaiting the callback before reading further records. ImageProgress and ImageProgressDetail are serializable data classes with explicit nullable fields. Counts are ULong values nested in progressDetail; only aux uses JsonObject. Unknown JSON fields are ignored; invalid typed fields return an error before the callback.
