...
- Each operation has an overload with a required suspending `onProgress(ImageProgress)` callback; existing signatures remain available. Non-error records are delivered in wire order, without an internal queue, awaiting the callback before reading further records. The raw JSON and optional layer/status/stream/progress/count/aux fields remain available.
