...
class ImageProgressTest {
    ...
    @Test fun countsRemainExactAcrossPlatformsAndUnknownFieldsAreIgnored() {
        val progress = json.decodeFromString<ImageProgress<Unit>>("""{"id":"layer","progressDetail":{"current":9007199254740993,"total":18446744073709551615,"future":true},"unknown":{"value":true}}""")
        val expected = ImageProgress<Unit>(id = "layer", progressDetail = ImageProgressDetail(9007199254740993uL, ULong.MAX_VALUE))
        assertEquals(expected, progress)
        assertEquals(expected, json.decodeFromString<ImageProgress<Unit>>(json.encodeToString(progress)))
        assertEquals("done", progress.copy(status = "done").status)
    }

    @Test fun absentCountsRemainUnknown() {
        assertEquals(ImageProgress<Unit>(), json.decodeFromString<ImageProgress<Unit>>("{}"))
        for (detail in listOf("null", "{}", "{\"current\":null,\"total\":null}")) {
            val progress = json.decodeFromString<ImageProgress<Unit>>("{\"progressDetail\":$detail}")
            assertNull(progress.progressDetail?.current)
            assertNull(progress.progressDetail?.total)
        }
    }

    @Test fun pushAuxUsesItsOperationTypeAndWireNames() {
        val progress = json.decodeFromString<ImageProgress<ImagePushResult>>("""{"aux":{"Tag":"latest","Digest":"sha256:result","Size":9007199254740993,"future":true}}""")
        assertEquals(ImagePushResult("latest", "sha256:result", 9007199254740993uL), progress.aux)
        assertEquals(progress, json.decodeFromString<ImageProgress<ImagePushResult>>(json.encodeToString(progress)))
        assertNull(json.decodeFromString<ImageProgress<ImagePushResult>>("{}").aux)
        assertFailsWith<SerializationException> {
            json.decodeFromString<ImageProgress<ImagePushResult>>("""{"aux":{"Size":-1}}""")
        }
    }
    ...
    @Test fun invalidCountsAreRejected() {
        for (detail in listOf("{\"current\":-1}", "{\"total\":1.5}", "{\"current\":\"invalid\"}", "{\"total\":18446744073709551616}")) {
            assertFailsWith<SerializationException> {
                json.decodeFromString<ImageProgress<Unit>>("{\"progressDetail\":$detail}")
            }
        }
    }
}
