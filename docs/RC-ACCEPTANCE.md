...
- [x] All four Docker/JDK PR matrix cells pass on the exact commit to be released; each includes JVM, Node.js and Linux X64 tests, samples and Dokka. Record run URL and SHA below.
...
Dashboard UI and application acceptance are not SDK release gates, as clarified by the owner on 2026-09-13. The library is validated directly through Kotlin integration tests against Docker. The [dashboard example](DASHBOARD.md) may be smoke-tested independently.

## Publication checks

- [x] Review migration notes and API snapshot changes, merge the approved MR, and verify checks on the intended release commit.
- [x] Create the candidate tag only when outstanding gates are resolved or the owner explicitly revises release scope. Do not move existing tags or republish a Maven version.
- [x] Release CI succeeds, including publication; GitHub release is marked prerelease for 1.0.0-rc.
- [x] Resolve all four published Maven artifacts (common/JVM/JS/Linux X64) from a fresh consumer and record the exact coordinates.
...
- Publication on 2026-09-13: MR #5 merged as 3906958a42a76624c28ab398b223b1c3cbf730ad (same tree as c278d8ff95312525fbc086c0a8a1a616066e4262, all four cells passed in run 34746157163). Tag v1.0.0-rc and GitHub prerelease are public. Release CI 34747664859 passed build, tests, both isolated acceptance suites and Maven deployment. Central deployment 65781810-2e71-4fb1-ba85-855d77ca0499 validated. Independent Gradle consumer resolution passed on 2026-09-13 with an initially empty dependency cache: dev.limebeck.libs:docker-client:1.0.0-rc, docker-client-jvm:1.0.0-rc, docker-client-js:1.0.0-rc and docker-client-linuxx64:1.0.0-rc (all in the same group), including transitive dependencies.
