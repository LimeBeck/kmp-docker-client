<p align="center">
  <img src="docs/assets/kmp-docker-client-logo.svg" alt="KMP Docker Client" width="680">
</p>
...
`1.1.0` adds Compose discovery and multi-service logs to the existing Maven artifact. Release publication is in progress; see the [Compose guide](docs/COMPOSE.md).
...
The [Compose API in 1.1.0](docs/COMPOSE.md), included in `lib`, adds discovery and logs for existing Compose projects on JVM, NodeJS and Linux X64, using the same Docker client and endpoint.
...
Use JDK 21 for development. The repository's default version is `1.1.0`; this setting does not publish artifacts. Versions live in `gradle/libs.versions.toml`. After changing Kotlin/JS dependencies, run `./gradlew kotlinUpgradeYarnLock` and commit the lockfile.
